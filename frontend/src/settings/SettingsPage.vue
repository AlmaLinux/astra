...
function toggleOtpQrCode(): void {
  document.getElementById("otp-qrcode")?.classList.toggle("d-none");
}
          ...
          <div class="tab-content">
            ...
            <div class="tab-pane" :class="{ active: activeTab === 'security' }" data-settings-tab-pane="security" role="tabpanel">
              ...
              <form method="post" novalidate :action="bootstrap.submitUrl">
                ...
                <div v-if="payload.security.usingOtp" id="otpinput" class="form-group"><label :for="findField(payload.security.password.form, 'otp')?.id">OTP code</label><input :id="findField(payload.security.password.form, 'otp')?.id || 'id_otp'" name="otp" type="text" autocomplete="one-time-code" :class="fieldClass(findField(payload.security.password.form, 'otp'))" :value="stringValue(findField(payload.security.password.form, 'otp'))"><div v-if="fieldErrors(findField(payload.security.password.form, 'otp')).length" class="invalid-feedback d-block"><div v-for="errorItem in fieldErrors(findField(payload.security.password.form, 'otp'))" :key="errorItem">{{ errorItem }}</div></div><small class="form-text text-muted">Your account has OTP enabled; enter your current OTP.</small></div>
