...
test("membership-profile-membership-card-links expose request-membership navigation", async ({ page }) => {
  ...
  await page.goto(resetState.routes.profiles.regular01);
...
test("membership-profile-pending-links surfaces the seeded pending and on-hold request links", async ({ page }) => {
  ...
  await loginViaForm(page, "regular04", "password");
  await page.goto(resetState.routes.profiles.regular04);
  await expect(page.locator("[data-user-profile-root]")).toBeVisible();
  ...
  await loginViaForm(page, "regular05", "password");
  await page.goto(resetState.routes.profiles.regular05);
  await expect(page.locator("[data-user-profile-root]")).toBeVisible();
  await expect(page.locator("[data-user-profile-membership-root]")).toBeVisible();
...
test("membership-organization-target-detail-links exposes the requested-for organization link and response links", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: "mirror.regular01-org.example.test", exact: true })).toHaveAttribute(
    "href",
...
test("membership-resubmit-on-hold-mirror validates and resubmits an on-hold mirror request", async ({ page }) => {
  ...
  const domainField = page.getByLabel(/domain name of the mirror/i);
  const pullRequestField = page.getByLabel(/link to your pull request/i);
  await expect(domainField).toHaveValue("https://mirror.regular04.example.test");
  ...
  await domainField.fill("https://mirror.regular04.example.test");
  await pullRequestField.fill("https://github.com/AlmaLinux/mirrors/pull/445");
  await page.getByLabel(/additional information/i).fill("Updated mirror evidence for committee review.");
  await page.getByRole("button", { name: /submit request/i }).click();
  await expect(page.getByText("Pending", { exact: true })).toBeVisible();
});
...
test("membership-rescind-pending-individual rescinds a pending request from the self-service detail route", async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/user\/regular05\/?$/);
