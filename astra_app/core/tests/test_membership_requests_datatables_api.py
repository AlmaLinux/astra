...
class MembershipRequestsDataTablesApiTests(TestCase):
    ...
    def test_membership_requests_page_renders_vue_asset_hook_when_vue_route_enabled(self) -> None:
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'data-membership-requests-root')
        self.assertContains(response, 'src="http://localhost:5173/@vite/client"')
        self.assertContains(response, 'src="http://localhost:5173/src/entrypoints/membershipRequests.ts"')
    ...
    def test_membership_requests_page_renders_datatables_shell(self) -> None:
        ...
        self.assertContains(response, 'data-membership-requests-next-url="/membership/requests/?filter=renewals&amp;pending_page=2&amp;on_hold_page=3"')
