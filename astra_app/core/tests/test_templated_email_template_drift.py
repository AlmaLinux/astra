...
class TemplatedEmailTemplateDriftTests(TestCase):
    ...
    @override_settings(ELECTION_VOTE_RECEIPT_EMAIL_TEMPLATE_NAME="vote-receipt-drift-test")
    def test_vote_receipt_template_missing_manifest_digest_logs_warning(self) -> None:
        from post_office.models import EmailTemplate
        ...
        EmailTemplate.objects.create(
            name="vote-receipt-drift-test",
            subject="Receipt {{ ballot_hash }}",
            content="Receipt {{ ballot_hash }} Nonce {{ nonce }} Weight {{ weight }}",
            html_content="<p>Receipt <code>{{ ballot_hash }}</code></p><p>{{ chain_hash }}</p><p>{{ previous_chain_hash }}</p>",
        )
        ...
        with (
            ...
            queue_templated_email(
                ...
                context={
                    ...
                    "weight": 1,
                    "chain_hash": "c" * 64,
                    "previous_chain_hash": "p" * 64,
                    "config_manifest_sha256": "d" * 64,
                },
            )

        self.assertTrue(
            any("missing required" in line.lower() and "config_manifest_sha256" in line.lower() for line in logs.output),
            f"expected warning about missing config_manifest_sha256, got: {logs.output}",
        )
